...
public class BusinessData {
    ...
    public static int getCurrentDevCompanyID(boolean passLocal, IceClient iceClient) {
        if (app!=null){
            SharedPreferences sp = ApplicationDevInfo.sharedStorage(app);
            if ( sp != null) {
                String devToken = ApplicationDevInfo.getShareDEVID(app)+"@"+ WebApplication.DEVTYPE;
                try {
    ...
    // 根据设备token获取公司ID
    private static int _getCompanyIDByToken(SharedPreferences sp, String devToken, boolean passLocal, IceClient iceClient) throws IOException {
        // 是否通过后台获取
